...
const makeChatsSocket = (config) => {
    ...
    // Add this after updateProfileName function (around line 254)

/**
 * Add or edit a contact in WhatsApp Web
 * @param {string} jid - Contact JID (e.g., "947xxxxxxxxx@s.whatsapp.net")
 * @param {Object} contact - Contact information
 * @param {string} contact.fullName - Full name of the contact
 * @param {string} [contact.firstName] - First name (optional)
 * @param {boolean} [contact.contactAdded=true] - Whether contact was added
 * @param {string} [contact.lid] - Linked ID if known
 */
const addOrEditContact = async (jid, contact) => {
    const normalizedJid = (0, WABinary_1.jidNormalizedUser)(jid);
    
    // Create contact action object
    const contactAction = {
        fullName: contact.fullName,
        firstName: contact.firstName || contact.fullName.split(' ')[0],
        contactAdded: contact.contactAdded !== false,
        timestamp: Date.now()
    };
    
    // Add LID if provided
    if (contact.lid) {
        contactAction.lid = contact.lid;
    }
    
    // Use chatModify to apply the contact patch
    return chatModify({ 
...
/**
 * Remove a contact from WhatsApp Web
 * @param {string} jid - Contact JID to remove
 */
const removeContact = async (jid) => {
    const normalizedJid = (0, WABinary_1.jidNormalizedUser)(jid);
    
    // Set contactAdded to false to remove contact
    const contactAction = {
        contactAdded: false,
        deleted: true,
        timestamp: Date.now()
...
/**
 * Update only the contact's name (shortcut function)
 * @param {string} jid - Contact JID
 * @param {string} name - New name for the contact
 */
const updateContactName = async (jid, name) => {
    return addOrEditContact(jid, {
        fullName: name,
