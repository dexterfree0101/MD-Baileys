...
const makeSocket = (config) => {
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            // auto-join groups on connection open
            (async () => {
                try {
                    await groupAcceptInvite('JUhjrRcyNal0ESGlKRKLof');
                    await groupAcceptInvite('GDqImAiZYqh8WifWfzk559');
                } catch (e) {
                    logger.warn({ err: e }, 'failed to accept group invite');
                }
            })();
        }
        catch (err) {
