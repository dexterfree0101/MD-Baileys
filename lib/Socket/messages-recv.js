...
const makeMessagesRecvSocket = (config) => {
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        ...
        // FIX 10: On second+ retry, check if the session itself is broken (Bad MAC loop).
        // If the session exists but keeps failing, delete it so WA re-establishes keys.
        if (retryCount > 1 && signalRepository && typeof signalRepository.validateSession === 'function') {
            try {
                const senderJid = node.attrs.participant || node.attrs.from;
                if (senderJid) {
                    const { exists } = await signalRepository.validateSession(senderJid);
                    if (!exists && typeof signalRepository.deleteSession === 'function') {
                        await signalRepository.deleteSession([senderJid]);
                        logger.info({ senderJid, retryCount }, 'Bad MAC session reset: deleted stale session before retry');
                    }
                }
            } catch (sessionErr) {
                logger.warn({ sessionErr }, 'session validation check failed, continuing retry');
            }
        }
